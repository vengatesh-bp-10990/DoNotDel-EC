...
function Signup() {
  const { isAuthenticated, user, loginUser } = useApp();
  ...
  // Render Catalyst Google sign-in button in a small container
  useEffect(() => {
    if (googleInitRef.current || isAuthenticated) return;
    const initGoogle = () => {
      if (window.catalyst?.auth?.signIn && googleContainerRef.current) {
        googleInitRef.current = true;
        window.catalyst.auth.signIn('catalyst-google-signup', { service_url: '/' });
      } else {
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg border border-gray-100 p-5 sm:p-7 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" name="name" value={form.name} onChange={handleChange}
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 focus:border-teal-400 focus:ring-2 focus:ring-teal-100 outline-none transition-all text-sm" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} name="password" value={form.password} onChange={handleChange}
                  ...
                  className="w-full pl-10 pr-11 py-2.5 rounded-xl border border-gray-200 focus:border-teal-400 focus:ring-2 focus:ring-teal-100 outline-none transition-all text-sm" />
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 shadow-lg shadow-emerald-200 hover:shadow-emerald-300 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {loading ? (
                <>
            ...
            {/* Google Sign-In via Catalyst (small embedded container) */}
            <div id="catalyst-google-signup" ref={googleContainerRef} className="w-full min-h-[60px] flex items-center justify-center" />
          </form>
